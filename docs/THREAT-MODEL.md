...
| **Catalog integrity** | Poisoning, traversal, impersonation | Route templates are validated (traversal, protocol smuggling, unparseable percent-encoding are hard drops), and descriptions containing anything that could be read as markup are refused outright rather than stripped (#217). | Ours | Edge cases in URI parsing might still bypass route-template validation. Descriptions are refused rather than escaped, so legitimate prose containing markup is dropped with the listing marked `partially landed`. |
