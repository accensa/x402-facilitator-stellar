...
| `invalid_declaration` | `rejected` | The declaration was not an object at all — the cataloging path received a null, a string, or a primitive where a discovery declaration was expected. | Send a JSON object shaped like the Seller Guide's example; validate offline with `node src/sdk/cli.js metadata.json` before paying again. |
| `invalid_extension_schema` | `rejected` | The `bazaar` extension in the payment payload does not conform to the upstream spec. | Validate offline with `node src/sdk/cli.js metadata.json` and fix the extension shape, then pay again. |
